...
static enum hrtimer_restart timer_callback(struct hrtimer* p_timer) {
	...
	uint64_t curr_tim = ktime_get_ns();
	uint8_t curr_gpio = ps->gpio_no;
	bool curr_on = ps->on;
	...
	if(!ps->on && !ps->down){
		...
		hrt_ret = hrtimer_forward_now(&ps->timer, ps->on_interval);
		//printk("hrt_ret %d\n", hrt_ret);
		do_nothing();
	}else{
		...
		hrt_ret = hrtimer_forward_now(&ps->timer, ps->off_interval);
		//printk("hrt_ret %d\n", hrt_ret);
		do_nothing();
